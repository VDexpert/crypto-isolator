...
def scraper_btc():
    ...
    id_obj = "A value the model object AnalyzerBTC that is automatically assigned when a command " \
    ...
    of the "url" field of the "AnalyzerBTC" model object
    '''
    ...
    with open('proxies.txt', encoding='utf-8') as f:
        ...
        while timing > 0:
            try:
                ...
                for elem in elems: #TODO edit file entry with name-pattern "result_analyzer_btc_id_object_analyzerBTC"
                    if elem.text:
                        print(f'{elem.text},{timezone.now()}')
